module.exports = function(){
    ...
    function getFavors(res, mysql, context, complete){
        mysql.pool.query("SELECT favor.creature_id1, favor.creature_id2 FROM favor", function(error, results, fields){
            if(error){
    ...
    router.get('/:creature_id1/creature_id2', function(req, res){
        callbackCount = 0;
        var context = {};
        context.jsscripts = ["selectedcreature.js", "updatecreature.js"];
        var mysql = req.app.get('mysql');
        getFavor(res, mysql, context, req.params.creature_id1, req.params.creature_id2, complete);
        getCreatures(res, mysql, context, complete);
        function complete(){
            callbackCount++;
            if(callbackCount >= 2){
    ...
    router.put('/:creature_id1/creature_id2', function(req, res){
        var mysql = req.app.get('mysql');
        var sql = "UPDATE favor SET creature_id1=?, creature_id2=? WHERE creature_id1=? AND creature_id2=?";
        var inserts = [req.body.creature_id1, req.body.creature_id2, req.params.creature_id1, req.params.creature_id2];
        sql = mysql.pool.query(sql,inserts,function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
    ...
    router.delete('/:creature_id1/creature_id2', function(req, res){
        var mysql = req.app.get('mysql');
        var sql = "DELETE FROM creatures WHERE creature_id1 = ? AND creature_id2 = ?";
        var inserts = [req.params.creature_id1, req.params.creature_id2];
        sql = mysql.pool.query(sql, inserts, function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.status(400);
